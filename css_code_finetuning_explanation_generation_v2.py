...
# !pip install transformers datasets torch pandas huggingface_hub
...
import getpass  # To securely prompt for the API token

# 0. Prompt for Hugging Face API Token
hf_token = getpass.getpass("Enter your Hugging Face API token: ")  # Securely prompt the user for the token
login(token=hf_token)  # Log in to Hugging Face using the provided token

# 1. Load the dataset (you can replace the path with your actual CSV dataset path)
df = pd.read_csv("HCUI.csv")  # Replace with correct file path
...
tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-2-7b")  # Correct model name
model = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-2-7b")  # Correct model name
